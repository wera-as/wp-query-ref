 ...
 * Source: https://core.trac.wordpress.org/browser/tags/5.6.3/src/wp-includes/query.php
 */
...
$args = [
  ...
  'author_name'     =>  'weraas',   // (string) - use 'user_nicename' (NOT name)
  'author__in'      =>  [2, 6],   // (array) - use author id (available with Version 3.7).
  'author__not_in'  =>  [2, 6],   // (array)' - use author id (available with Version 3.7).
  ...
  'post_name__in'           =>  ['sample-post-a', 'sample-post-b'],   // - use post slugs. Specify posts to retrieve. (available since Version 4.4)
  'post_parent'             =>  1,                                     // (int) - use page id. Return just the child Pages. (Only works with heirachical post types.)
  'post_parent__in'         =>  [1, 2, 3],                             // (array) - use post ids. Specify posts whose parent is in an array. NOTE: Introduced in 3.6
  ...
  'has_password'    =>   true,            // (bool) - available with Version 3.9, true for posts with passwords, false for posts without passwords, null for all posts with and without passwords
  ...
  'ignore_sticky_posts'     =>  false,                        // (boolean) - ignore sticky posts or not (available with Version 3.1, replaced caller_get_posts parameter). Default value is 0 - don't ignore sticky posts.
  ...
  'post_mime_type'     =>   'image/gif',                    // (string) - Allowed mime types.
  'post_mime_type'     =>   ['image/gif', 'image/png'],   // (array) - Allowed mime types.
  ...
  'no_found_rows'             =>  false,    // (bool) Default is false. WordPress uses SQL_CALC_FOUND_ROWS in most queries in order to implement pagination. Even when you don’t need pagination at all. By Setting this parameter to true you are telling wordPress not to count the total rows and reducing load on the DB. Pagination will NOT WORK when this parameter is set to true.
  ...
  'fields'    =>  'ids',  // (string) - Which fields to return.
  // Possible values:
